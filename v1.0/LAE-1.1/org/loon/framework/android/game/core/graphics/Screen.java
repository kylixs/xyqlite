...
public abstract class Screen implements LInput, LRelease {
	...
	public void onCreate(int width, int height) {
		...
		this.isDraging = isLoad = isLock = isClose = isGravityToKey = false;
		this.isNext = true;
	}
     ...
     * @return
     */
    public LComponent findComponent(int x, int y){
        if(desktop != null){
            LComponent comp = desktop.getContentPane().findComponent(x, y);
            if(comp != desktop.getContentPane()){
                return comp;
            }
        }
        return null;
