...
import com.androidxyq.item.ItemManager;
...
public class CommandInterpreter {
	...
	public CommandInterpreter(BattleScreen canvas) {
		this.canvas = canvas;
	}
	
	public boolean exec(Command cmd) {
        System.err.println("thread: "+Thread.currentThread().getId()+"  Exec Command: "+cmd);
		try {
	...
	public void attack(Command cmd) {
        ...
        //TODO 修正相对位置计算
        Sprite s = source.getCacheSprite("person", "attack");
		int dx = Math.abs(s.getWidth()-s.getRefPixelX());
		int dy = Math.abs(s.getHeight()-s.getRefPixelY());
		if(target.getX() > source.getX()) {
			dx = -dx;
			dy = -dy;
		}
        System.err.println("attact: source:("+source.getX()+","+source.getY()+"), target:("+target.getX()+","+target.getY()+"), offset: ("+dx+","+dy+")");
		...
		if(hit) {
			...
			if(cmd.get("defend")!=null) {
				...
				target.playEffect("defend", false);
			}else {
				target.playOnce("hit");
				target.playEffect("hit", false);
			}
			source.waitFor();
		}else {
			backward(target);
		...
		//检查目标角色的状态并处理
        checkRoleStatus(target, oldState);
        setMsg("");
		hidePoints(target);
	}
     ...
     * @param oldState
     */
    private void checkRoleStatus(Player target, String oldState) {
        if(target.getData().hp <=0) {
            //target.playOnce("die");
            target.getData().hp = 0;
            System.out.printf("%s招架不住，倒在战场上。\n",target.getName());
            //判断如果是英雄，则切换到死亡状态
            if(target == canvas.getHero()){
                target.playOnce("die");
                //TODO 修复精灵waitFor()
                //target.waitFor();
                try {
                    Thread.sleep(target.getPerson().getAnimationCount()*100);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }else {
                canvas.cleanRole(target);
            }
        }else {
