...
import com.androidxyq.item.Item;
import com.androidxyq.item.ItemInstance;
import com.androidxyq.item.MedicineItem;
...
public class BattleCalculator {
	...
	private Map<Player,PlayerStatus> datas = new HashMap<Player, PlayerStatus>();
	...
	public List calc(List<Command> cmdlist,List ownsideTeam,List adversaryTeam,Map params) {
		...
		for (int i = 0; i < ownsideTeam.size(); i++) {
			...
			PlayerStatus vo = new PlayerStatus(player.getData());
			datas.put(player, vo);
		}
	...
	public Command attack(Command cmd) {
		PlayerStatus src = datas.get(cmd.getSource());
		PlayerStatus target = datas.get(cmd.getTarget());
		if(target == null) {
