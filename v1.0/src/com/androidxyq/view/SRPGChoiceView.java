...
public class SRPGChoiceView extends SRPGView {
	...
	public SRPGChoiceView(String[][] messages, int[] joint, LFont font,
			int x, int y) {
		set(messages, joint, font, x, y);
	}

	public void set(String[] messages, int[] joint, LFont font, int x,
			int y) {
		set(choiceFormat(messages), joint, font, x, y);
	}
	 ...
	 * @param y
	 */
	public void set(LImage image, String[][] messages, int[] joint, LFont font,
			int x, int y) {
		super.exist = true;
	 ...
	 * @return
	 */
	private synchronized LImage createDefaultDialog(int w, int h) {
		if (lazyDialog == null) {
		...
		if (o == null) {
			o = LImage.createImage(w, h, Config.ARGB_8888);
		...
		try {
			...
			for (;;) {
				...
				for (int j1 = 0; j1 < choice[viewIndex].length; j1++) {
					g.drawStyleString(choice[viewIndex][j1], posX + i1 + 14,
							posY + index * getSpace() + getHeight(),
							LColor.darkGray, nColor);
					i1 += mesList[j1] + tab;
				}
				index++;
			}
